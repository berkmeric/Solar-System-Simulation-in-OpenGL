...
class MeshDrawer {
	...
	draw(matrixMVP, matrixMV, matrixNormal, modelMatrix) {
		...
		var uLightPosLoc = gl.getUniformLocation(this.prog, 'uLightPos');
...
void main()
{
	...
	vec3 diffuseColor = diff * texture2D(tex, vTexCoord).rgb;
	vec3 ambientColor = ambient * texture2D(tex, vTexCoord).rgb;
